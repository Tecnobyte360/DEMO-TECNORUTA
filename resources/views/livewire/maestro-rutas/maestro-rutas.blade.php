        ...
        <h4 class="text-lg font-semibold text-gray-700 dark:text-gray-300">Asignar Inventario</h4>
                            ...
                            <div class="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                @foreach($asignaciones as $index => $item)
                                    <div class="relative bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 
                                        ...
                                        <button wire:click="eliminarAsignacion({{ $index }})"
            ...
            @if ($isEdit)
                <button type="button" wire:click="actualizarRuta"
                    class="flex items-center gap-2 px-6 py-3 bg-yellow-600 hover:bg-yellow-700 text-white text-base font-bold rounded-2xl shadow-md hover:shadow-lg transition-all"
                ...
                @forelse($rutas as $ruta)
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        ...
                        <td class="p-3 text-center space-x-2">
                            ...
                            <button onclick="confirmarEliminacion({{ $ruta->id }})" class="text-red-600 hover:text-red-800" title="Eliminar">
                                <i class="fas fa-trash-alt"></i>
                            </button>
                            <button type="button" wire:click="verInventario({{ $ruta->id }})"
                                class="text-gray-600 hover:text-indigo-700" title="Ver inventario asignado">
...
{{-- SweetAlert scripts --}}
<script>
    window.addEventListener('ruta-creada', event => {
        Swal.fire({ icon: 'success', title: '¡Ruta registrada!', text: event.detail.mensaje, timer: 2000, showConfirmButton: false });
    });
    window.addEventListener('ruta-actualizada', event => {
        Swal.fire({ icon: 'success', title: '¡Actualizado!', text: event.detail.mensaje, timer: 2000, showConfirmButton: false });
    });
    window.addEventListener('ruta-eliminada', event => {
        Swal.fire({ icon: 'success', title: '¡Eliminada!', text: event.detail.mensaje, timer: 2000, showConfirmButton: false });
    });
    window.addEventListener('error', event => {
        Swal.fire({ icon: 'error', title: 'Error', text: event.detail.mensaje });
    });
    {{-- SweetAlert Script Listener --}}
<script>
    window.addEventListener('error', event => {
        Swal.fire({
            icon: 'error',
